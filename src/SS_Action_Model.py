...
class SS_Action_Model:
	...
	def add_conf_noise(self, traj,sigma=0.25):
		conf = traj[1]
		noisy_conf = []
		for q in conf:
			nq = np.random.normal(loc=q,scale=sigma)
			noisy_conf.append(conf)
		return noisy_conf
	...
	def execute_action(self, action):
		...
		try:
			if action.name == 'open-drawer':
				...
				# self.robot.plan_and_drive_to_pose(current_pose)
				self.robot.raise_arm_after_pick()
				pose = (action.pfs['Pose'],p.getQuaternionFromEuler((0,1.57,0)))
				trajectory = self.robot.plan_arm_motion(pose, 'right_arm')
				trajectory = self.add_conf_noise(trajectory)
				...
				self.robot.plan_and_drive_to_pose(self.world.sink_base_pose)
				# trajectory = action.maps['Trajectory']
				pose = (action.pfs['Pose'],p.getQuaternionFromEuler((1.57,0,0)))
				trajectory = self.robot.plan_arm_motion(pose, 'right_arm')
				trajectory = self.add_conf_noise(trajectory)
				...
				eepose = pyplan.get_link_pose(self.robot.id, self.robot.arms_ee['right_arm'])
				obpose = pyplan.get_point(self.world.mug)
				dist = np.linalg.norm((np.array(eepose[0])[:2] - np.array(obpose)[:2]))
